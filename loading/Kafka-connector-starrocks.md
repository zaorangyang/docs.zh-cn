...
| key.converter                       | 否       | StarRocks Kafka Connector默认使用kafka connect集群的key converter, 你可以通过这个配置项配置StarRocks Kafka Connector使用的key converter。                                                            | 在本场景中 StarRocks 提供的 Kafka connector 是sink connector，该参数表示 sink connector 的 key converter，用于反序列化 Kafka 数据的 key。您需要根据 source connector 的 key converter 确定该 key converter。 |
| value.converter                     | 否       | StarRocks Kafka Connector默认使用kafka connect集群的value converter, 你可以通过这个配置项配置StarRocks Kafka Connector使用的value converter。                                                             | sink connector 的 value converter，用于反序列化 Kafka 数据的 value。您需要根据 source connector 的 value converter 确定该 value converter。 |
