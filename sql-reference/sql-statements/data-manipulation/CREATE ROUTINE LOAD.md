...
| task_consume_second                 | 否           | Routine Load 导入作业中每个 Routine Load task 消费数据的最大时长，单位为秒。当未配置`task_consume_second` 和 `task_timeout_second` 时，StarRocks 则使用 FE 动态参数 `routine_load_task_consume_second `和`routine_load_task_timeout_second` 来控制导入行为。当只配置 `task_consume_second` 时，默认 `task_timeout_second` = `task_consume_second` * 4。当只配置 `task_timeout_second` 时，默认 `task_consume_second` = `task_timeout_second` /4|
| task_timeout_second                 | 否           | 该Routine Load job, 每个 Routine Load task 超时时间，单位为秒。<>当未配置task_consume_second和task_timeout_second时，StarRocks 则使用 `routine_load_task_consume_second` 和 `routine_load_task_timeout_second` 来控制导入行为。当只配置 `task_timeout_second` 时，默认 `task_consume_second` = `task_timeout_second` /4。当只配置 `task_consume_second` 时，默认 `task_timeout_second` = `task_consume_second` * 4。|
