...
| task_consume_second                 | 否           | 该Routine Load job, 每个 Routine Load task 消费数据的最大时间，单位为秒。当未配置task_consume_second和task_timeout_second时，SR将使用routine_load_task_consume_second和routine_load_task_timeout_second来控制导入行为。当只配置task_consume_second时，默认task_timeout_second = task_consume_second*4。|
| task_timeout_second                 | 否           | 该Routine Load job, 每个 Routine Load task 超时时间，单位为秒。当未配置task_consume_second和task_timeout_second时，SR将使用routine_load_task_consume_second和routine_load_task_timeout_second来控制导入行为。当只配置task_timeout_second时，默认task_consume_second = task_timeout_second/4。|
